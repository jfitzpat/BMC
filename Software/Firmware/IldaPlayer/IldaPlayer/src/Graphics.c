...
// Internal Functions
static void FillBuffer(void *pDst, uint32_t xSize, uint32_t ySize, uint32_t OffLine, uint32_t ColorIndex);
...
void graphics_SetTextColor(uint32_t newcolor)
{
	TextColor = newcolor;
}

uint32_t graphics_GetTextColor()
{
	return TextColor;
}
...
void graphics_CopyBuffer(uint32_t *pSrc, uint32_t *pDst, uint16_t x, uint16_t y, uint16_t xsize, uint16_t ysize)
{
...
void graphics_DrawRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height)
{
...
void graphics_FillRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height)
{
  uint32_t  Xaddress = 0;

  Xaddress = TargetAddress + 4*(800*Ypos + Xpos);
  FillBuffer((uint32_t *)Xaddress, Width, Height, (800 - Width), TextColor);
}
...
void graphics_DrawHLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length)
{
  ...
  Xaddress = TargetAddress + 4*(800*Ypos + Xpos);
  FillBuffer((uint32_t *)Xaddress, Length, 1, 0, TextColor);
}
...
void graphics_DrawVLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length)
{
  ...
  Xaddress = TargetAddress + 4*(800*Ypos + Xpos);
  FillBuffer((uint32_t *)Xaddress, 1, Length, (800 - 1), TextColor);
}
...
void graphics_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
  ...
  {
    graphics_DrawPixel(x, y, TextColor);   	  /* Draw the current pixel */
...
void graphics_DrawBitmap(uint32_t Xpos, uint32_t Ypos, uint8_t *pbmp)
{
  ...
  /* Set the address */
  Address = TargetAddress + (((800*Ypos) + Xpos)*(4));
...
{
  *(__IO uint32_t*) (TargetAddress + (4*(Ypos*800 + Xpos))) = RGB_Code;
}

void graphics_DisplayChar(uint16_t Xpos, uint16_t Ypos, uint8_t Ascii)
{
  DrawChar(Xpos, Ypos, &Font->table[(Ascii-' ') *
    Font->Height * ((Font->Width + 7) / 8)]);
}

void graphics_DisplayStringAt(uint16_t Xpos, uint16_t Ypos, uint8_t *Text, GRAPHICS_ALIGN_MODE Mode)
{
  ...
  while ((*Text != 0) & (((800 - (i*Font->Width)) & 0xFFFF) >= Font->Width))
  {
  ...
  for(i = 0; i < height; i++)
  {
    ...
    for (j = 0; j < width; j++)
    {
      if(line & (1 << (width- j + offset- 1)))
        graphics_DrawPixel((Xpos + j), Ypos, TextColor);
      else
