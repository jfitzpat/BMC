...
class SpiralPopup : public Component,
                    public juce::Slider::Listener,
                    public juce::Button::Listener
{
public:
    SpiralPopup (FrameEditor* editor)
    : frameEditor (editor)
    {
        scaleXSlider.reset (new juce::Slider ("scaleXSlider"));
        addAndMakeVisible (scaleXSlider.get());
        scaleXSlider->setRange (-25, 25, 0.05);
        ...
        scaleYSlider->setRange (100, 60000, 100);
        ...
        scaleYSlider->setValue (20000, dontSendNotification);
        centerButton->setToggleState (true, dontSendNotification);
        
        frameEditor->startTransform ("Spiral Point(s)");
    }
    ...
    {
        float angle = (float)scaleXSlider->getValue();
        int esize = (int)scaleYSlider->getValue();
        bool b = centerButton->getToggleState();
        frameEditor->spiralIldaSelected(angle, esize, b, false);
    }
...
//==============================================================================
class SpiralButton : public PopupTool
{
public:
    SpiralButton (FrameEditor* editor)
    : PopupTool (editor)
    {;}
    
    std::unique_ptr<Component> makeComponent() override
    {
        return std::make_unique<SpiralPopup>(frameEditor);
    }
};
